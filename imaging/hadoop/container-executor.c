...
/*
  ...
  fprintf(LOGFILE, "got pwd \n");

  if (getpwnam_r(user, buffer, ((char*)buffer) + sizeof(struct passwd), string_size, &result) != 0) {
    free(buffer);
     fprintf(LOGFILE, "failed getpwnam_r \n");

    fprintf(LOGFILE, "Can't get user information %s - %s\n", user,
           strerror(errno));
    return NULL;
  }
  fprintf(LOGFILE, "name %s", buffer.pw_gecos);
  return result;
  */
...
int set_user(const char *user) {
  // free any old user
  fprintf(LOGFILE, "maybe free_user \n");  
  if (user_detail != NULL) {
    free(user_detail);
    user_detail = NULL;
  }
  fprintf(LOGFILE, "going to check user \n");  
...
static int open_file_as_nm(const char* filename) {
  fprintf(LOGFILE, "open_file_as_nm.\n");
  uid_t user = geteuid();
  gid_t group = getegid();
  if (change_effective_user(nm_uid, nm_gid) != 0) {
    return -1;
...
int initialize_user(const char *user, char* const* local_dirs) {
  fprintf(LOGFILE, "initialize_user.\n");
  char *user_dir;
  char* const* local_dir_ptr;
  int failed = 0;
...
int create_log_dirs(const char *app_id, char * const * log_dirs) {
  fprintf(LOGFILE, "create_log_dirs().\n");
  char* const* log_root;
...
int create_container_log_dirs(const char *container_id, const char *app_id,
                              char * const * log_dirs) {
  fprintf(LOGFILE, "create_container_log_dirs\n");
  char* const* log_root;
...
int initialize_app(const char *user, const char *app_id,
                   ...
                   char* const* args) {
  fprintf(LOGFILE, "initialize_app( \n");  
  if (app_id == NULL || user == NULL || user_detail == NULL || user_detail->pw_name == NULL) {
   ...
   fprintf(LOGFILE, "create user dirs \n");  
  ...
  if (log_create_result != 0) {
   ...
   fprintf(LOGFILE, "create container log \n");  
  ...
  fprintf(LOGFILE, "initialize_app running final execvp. See you on the other side. \n");
  execvp(args[0], args);
