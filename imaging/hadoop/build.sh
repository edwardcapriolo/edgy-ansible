...
cat << EOF > Dockerfile
  FROM hadoop-base-build AS hadoop-build
  ...
  COPY exception.c hd_src/hadoop-common-project/hadoop-common/src/main/native/src/exception.c

  RUN --mount=type=cache,target=/root/.m2 cd /build/hd_src/hadoop-common-project/hadoop-common && mvn clean && mvn package -Denforcer.skip=true -Pnative -DskipTests -Dtar
  RUN cp /build/hd_src/hadoop-common-project/hadoop-common/target/native/target/usr/local/lib/* /usr/local/lib

  RUN --mount=type=cache,target=/root/.m2 cd /build/hd_src/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-server/hadoop-yarn-server-nodemanager && mvn package -Denforcer.skip=true -Pnative -DskipTests -Dtar
  #RUN cp /build/hd_src/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-server/hadoop-yarn-server-nodemanager/target/native/target/usr/local/bin/* /usr/local/bin
  RUN cp /build/hd_src/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-server/hadoop-yarn-server-nodemanager/target/native/target/usr/local/bin/container-executor /usr/local/bin/container-executor
